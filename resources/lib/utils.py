...
def log(s):
	print "[%s v%s] %s" % (config.NAME, version.VERSION, s)

def log_error(message=None):
	exc_type, exc_value, exc_traceback = sys.exc_info()
	if message:
		exc_value = message
	print "[%s v%s] ERROR: %s (%d) - %s" % (config.NAME, version.VERSION, exc_traceback.tb_frame.f_code.co_name, exc_traceback.tb_lineno, exc_value)
...
def dialog_error(msg):
	...
	content = []
	exc_type, exc_value, exc_traceback = sys.exc_info()
	content.append("%s v%s Error" % (config.NAME, version.VERSION))
...
def dialog_message(msg, title=None):
	if not title:
		title = "%s v%s" % (config.NAME, version.VERSION)
